...
document.addEventListener('keydown',e=>{if(e.key==='Escape'){forceCloseCertModal();closeRealisationReader();}});
...
function closeRealisationReader(){const modal=byId('realisation-reader');if(modal)modal.remove();document.body.classList.remove('modal-open')}
window.closeRealisationReader=closeRealisationReader;
function openRealisationDetail(r){
  closeRealisationReader();
  const stack=(r.stack||r.tools||[]).map(t=>`<span class="tag">${safe(t)}</span>`).join('');
  const skills=(r.skills||[]).map(s=>`<span class="tag">${safe(s)}</span>`).join('');
  const features=(r.features||[]).map(x=>`<li>${safe(x)}</li>`).join('');
  const difficulties=(r.difficulties||[]).map(x=>`<li>${safe(x)}</li>`).join('');
  const results=(r.results||[]).map(x=>`<li>${safe(x)}</li>`).join('');
  const resources=(r.resources||[]).map(x=>`<a class="resource-link" href="${safe(x.url||'#')}">${safe(x.label)}${x.type?' — '+safe(x.type):''}</a>`).join('');
  const code=(r.codeSnippets||[]).map(c=>`<div class="code-block"><div class="code-title">${safe(c.title)} <span>${safe(c.language)}</span></div><pre><code>${escapeHtml(c.code)}</code></pre></div>`).join('');
  const screenshots=(r.screenshots||[]).map(img=>`<img src="${safe(img)}" alt="Capture du projet">`).join('');
  const meta=`<div class="real-meta"><span>${safe(r.level)}</span><span>${safe(r.status)}</span></div>`;
  const sidebar=`<aside class="real-reader-sidebar"><p class="sidebar-title">Project info</p>${meta}${stack?`<h4>Outils</h4><div class="tags">${stack}</div>`:''}${skills?`<h4>Compétences</h4><div class="tags">${skills}</div>`:''}${resources?`<h4>Ressources</h4><div class="resource-grid">${resources}</div>`:''}</aside>`;
  const extras=`${features?`<section class="doc-extra"><h3>Fonctionnalités</h3><ul>${features}</ul></section>`:''}${difficulties?`<section class="doc-extra"><h3>Difficultés rencontrées</h3><ul>${difficulties}</ul></section>`:''}${results?`<section class="doc-extra"><h3>Résultats obtenus</h3><ul>${results}</ul></section>`:''}${screenshots?`<section class="doc-extra"><h3>Captures</h3><div class="screenshot-grid">${screenshots}</div></section>`:''}${code?`<section class="doc-extra"><h3>Codes</h3>${code}</section>`:''}`;
  const modal=make('div','realisation-reader',`<div class="real-reader-backdrop" data-real-close="true"></div><section class="real-reader-panel" role="dialog" aria-modal="true"><header class="real-reader-header"><div><span class="card-kicker">${realisationLabel(r.type)} • ${safe(r.category)}</span><h2>${safe(r.title)}</h2><p>${safe(r.description)}</p></div><button class="real-reader-close" type="button" data-real-close="true">Fermer</button></header><div class="real-reader-layout">${sidebar}<article class="real-reader-document markdown-body obsidian-doc">${extras}${markdownToHtml(r.documentation||'')}</article></div></section>`);
  modal.id='realisation-reader';
  document.body.appendChild(modal);
  document.body.classList.add('modal-open');
  modal.querySelectorAll('[data-real-close="true"]').forEach(btn=>btn.addEventListener('click',closeRealisationReader));
}
