(function(){
  ...
  function slugify(value){
    return clean(value).toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/\.[a-z0-9]+$/i,'').replace(/[^a-z0-9._-]+/g,'-').replace(/^-+|-+$/g,'')||'item';
  }

  function normalizeProject(item){
    const title=clean(item.publicTitle||item.title||item.name||'Projet documenté');
    return{
      type:'project',
      title,
      description:clean(item.description||item.summary||'Projet technique documenté avec contexte, méthode, résultats et enseignements.'),
      category:clean(item.category||'Projet'),
      platform:clean(item.platform||''),
      tags:[...asList(item.skills),...asList(item.tools)].slice(0,4),
      slug:slugify(item.slug||item.id||item.path||item.obsidianPath||title)
    };
  }

  function normalizeWriteup(item){
    const title=clean(item.publicTitle||item.title||item.name||'Writeup technique');
    return{
      type:'writeup',
      title,
      description:clean(item.description||item.summary||'Analyse technique structurée : reconnaissance, exploitation, validation et mesures de sécurité.'),
      category:clean(item.category||'Writeup'),
      platform:clean(item.platform||item.sectionLabel||''),
      tags:[...asList(item.skills),...asList(item.tags)].slice(0,4),
      slug:slugify(item.slug||item.id||item.path||item.obsidianPath||title)
    };
  }
  ...
  function workCard(item,index){
    ...
    return `<article class="work-card">
      <div class="work-visual" data-kind="${item.type}"><span class="work-index">CASE / ${String(index+1).padStart(2,'0')}</span></div>
      <div class="work-card-body">
        <p class="work-meta">${esc(meta)}</p>
        <h3>${esc(item.title)}</h3>
        <p>${esc(item.description)}</p>
        ${tags?`<div class="work-tags">${tags}</div>`:''}
        <a class="work-link" href="${esc(href)}">Ouvrir le dossier</a>
      </div>
    </article>`;
  }
  ...
  async function hydrate(){
    ...
    try{
      ...
      setMetric('metric-projects',projects.length);
      setMetric('metric-writeups',writeups.length);
      setMetric('metric-certifications',certifications.length);
      setMetric('metric-skills',skills.length);
      setMetric('hero-projects',projects.length);
      setMetric('hero-writeups',writeups.length);
      setMetric('hero-certs',certifications.length);
      ...
      console.error(error);
      if(featured)featured.innerHTML='<p class="work-empty">Les réalisations sont temporairement indisponibles.</p>';
    }
  }
