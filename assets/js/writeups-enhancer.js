(function(){
  ...
  function slugFile(name){const n=safe(name).split('|')[0].trim().replace(/^\/+/, ''); if(n.includes('/')) return n.split('/').map(slugify).join('/'); const ext=(n.match(/\.[A-Za-z0-9]+$/)||[''])[0]; const base=ext?n.slice(0,-ext.length):n; return slugify(base)+ext.toLowerCase();}
  ...
  function firstPara(body){const clean=safe(body).replace(/^#\s+.*$/m,'').trim();const parts=clean.split(/\n\s*\n/).map(p=>p.trim()).filter(p=>p&&!p.startsWith('#')&&!p.startsWith('```')&&!p.startsWith('![')&&!p.startsWith('![['));return parts[0]||'Documentation publiée depuis Obsidian.'}
  ...
  function mediaSrc(url){
    let u=safe(url).split('|')[0].trim();
    if(!u) return '';
    if(/^(https?:|data:|blob:)/i.test(u)) return u;
    if(u.startsWith('/portofiolo/')) return assetPath(u);
    if(u.startsWith('/content/')) return assetPath(u);
    if(u.startsWith('content/')) return assetPath(u);
    if(u.startsWith('/assets/')) return assetPath(u);
    if(u.startsWith('assets/')) return assetPath(u);
    return assetPath('content/writeups/media/'+slugFile(u));
  }
  function mediaHtml(alt,url){
    const src=mediaSrc(url);
    const clean=safe(src).split('?')[0].split('#')[0].toLowerCase();
    const caption=esc(alt||safe(url).split('/').pop().split('|')[0]||'Pièce jointe');
    const srcEsc=esc(src);
    if(/\.(png|jpe?g|gif|webp|svg)$/i.test(clean)) return `<figure class="doc-media"><img src="${srcEsc}" alt="${caption}" loading="lazy" onerror="this.onerror=null;this.src=this.src.replace('/portofiolo/','/');"><figcaption>${caption}</figcaption></figure>`;
    if(/\.(mp3|wav|ogg|m4a|aac|flac)$/i.test(clean)) return `<figure class="doc-media"><audio controls src="${srcEsc}"></audio><figcaption>${caption}</figcaption></figure>`;
    if(/\.(mp4|webm|mov|mkv)$/i.test(clean)) return `<figure class="doc-media"><video controls src="${srcEsc}"></video><figcaption>${caption}</figcaption></figure>`;
    if(/\.pdf$/i.test(clean)) return `<p><a class="doc-attachment" href="${srcEsc}" target="_blank" rel="noopener">Ouvrir le PDF : ${caption}</a></p>`;
    return `<p><a class="doc-attachment" href="${srcEsc}" target="_blank" rel="noopener">Ouvrir la pièce jointe : ${caption}</a></p>`;
  }
  function linkHref(url){return /^(https?:|mailto:|tel:|#)/i.test(safe(url))?safe(url):assetPath(url)}

  function mdToHtml(md){let text=safe(md);const codes=[];text=text.replace(/```([a-zA-Z0-9_-]*)\n([\s\S]*?)```/g,(_,lang,code)=>{const token=`@@CODE${codes.length}@@`;codes.push(`<pre><code>${esc(code)}</code></pre>`);return token});text=esc(text);text=text.replace(/^### (.*)$/gm,'<h4>$1</h4>').replace(/^## (.*)$/gm,'<h3>$1</h3>').replace(/^# (.*)$/gm,'<h2>$1</h2>');text=text.replace(/!\[\[([^\]]+)\]\]/g,(_,ref)=>mediaHtml(ref,ref));text=text.replace(/!\[(.*?)\]\((.*?)\)/g,(_,alt,url)=>mediaHtml(alt,url));text=text.replace(/\[(.*?)\]\((.*?)\)/g,(_,label,url)=>`<a href="${esc(linkHref(url))}" target="_blank" rel="noopener">${label}</a>`);text=text.replace(/^\- (.*)$/gm,'<li>$1</li>');text=text.replace(/(<li>.*<\/li>\n?)+/g,m=>`<ul>${m}</ul>`);text=text.split(/\n\s*\n/).map(b=>b.trim()).filter(Boolean).map(b=>/^<(h\d|ul|pre|figure|p><a|img|video|audio)/.test(b)?b:`<p>${b.replace(/\n/g,'<br>')}</p>`).join('\n');codes.forEach((h,i)=>text=text.replace(`@@CODE${i}@@`,h));return text}
