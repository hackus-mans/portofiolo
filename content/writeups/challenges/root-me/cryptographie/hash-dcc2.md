...
Aussi appelé **MSCACHE**2 ou **DCC**2. Ce n'est pas une base de données de comptes, mais un "historique de secours".
...
```
/Administrator:$DCC2$10240#Administrator#23d97555681813db79b2ade4b4a6ff25
```
...
* Ensuite essayé de déterminé le type de hash que vous avez en face de vous avec la commande :
```bash
...
```bash
john --format=mscash2 --wordlist=/usr/share/wordlists/rockyou.txt hash-dcc2.txt 
```
